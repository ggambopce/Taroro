...
/**
 * 인메모리 이메일 인증
 * 이메일 임증코드를 애플리케이션 메모리에 임시 저장-검증하는 컴포넌트
 * key = 이메일, value = 코드, 만료시각, 검증여부
 * 사용 흐름
 * 1) save(email, code, ttl) 로 발급된 인증코드 저장
 * 2) 사용자가 입력한 코드로 checkAndMarkVerified(email, code) 호출 → 일치하면 verified=true
 * 3) 특정 시점에서 isVerified(email) 로 검증 완료 + 유효기간 내인지 확인
 * 4) @Scheduled 로 주기적으로 만료(cleanUpExpired) 데이터 제거
 *
 * ConcurrentHashMap 을 사용하여 다중 스레드(여러 요청)가 동시에 접근해도 안전하게 동작
 * 인터페이스로 확장하여 redis에서 기능구현 가능성 고려
 *
 */
@Slf4j
@Component
